    ...
    <x-mary-form wire:submit="save" class="p-6 lg:p-8 bg-white border-b border-gray-200">
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                <x-mary-select label="Doradca" icon-right="o-user" :options="$users" placeholder="Wybierz doradcę"
                    wire:model.live="adviser" />
            </div>
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                <x-mary-select label="Doradca polecający" icon-right="o-user" :options="$users"
                    placeholder="Wybierz doradcę" wire:model.defer="recommended" />
                @error('name')
        ...
        <div class="flex flex-wrap -mx-3 mb-6 gap-2">
            ...
            @foreach ($connections as $connection)
                ...
                <x-mary-checkbox label="{{ $connection->name }}" value="{{ $connection->id }}"
                    wire:model.defer="connection" id="{{ $connection->id }}" />
                {{--
            </div> --}}
            @endforeach
        </div>
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full md:w-1/6 px-3 mb-6 md:mb-0">
                ...
                @foreach ($deposits as $deposit)
                    <div>
                        <x-mary-checkbox class="mb-1.5 mt-1" label="{{ $deposit->name }}" wire:model.defer="deposit"
                            value="{{ $deposit->id }}" left />
